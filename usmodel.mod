 ...
 * Smets, Frank and Wouters, Rafael (2007): "Shocks and Frictions in US Business Cycle:
 * A Bayesian DSGE Approach", American Economic Review, 97(3), 586-606,
 * that are compatible with Dynare 4.5 onwards
 *
 * Notes:
 * - The original code released by Smets and Wouters contains several issues.
 *     Johannes Pfeifer has pointed out and corrected these issues, and made the code
 *     publicly available on his GitHub repository. The issues (summurized by Johannes Pfeifer) include:
 ...
 * 
 * - Compared with Johannes Pfeifers code, I made the following improvements:
 *       - In the usmodel.mod, the posterior mode is recomputed using Newton-type optimizer - `mode_compute=4`,
 *            rather than using the mode provided in the original code.
 *       - The code is fully self-contained and does not require any additional inputs to be downloaded from external
 *            external sources. It can be run directly using the provided `usmodel_data.mat`, or with any dataset covering
 *            a user-specified time period, as long as it follows the same format as `usmodel_data.mat`. See README.md for 
 *            details on the required variables and data format.
 *       - This file provides the code for computing recursive forecasts and RMSE, as well as generating formatted figures. 
 *            The corresponding scripts are `recursive_forecasts.m`, `plot_recursive_forecasts.m`, and `plot_rmse_recursive.m`, respectively.
 *       - This file also provides the code for computing and plotting IRFs.
 *            The corresponding scripts are `usmodel_irf.mod` and `plot_irf.m`.
 *
 * This file was originally written by Frank Smets and Rafeal Wouters, and has been updated by Johannes Pfeifer.
 * Now, further improvements are made by Yujia Ming.
 * 
 * Please note that the following copyright notice applies to all Dynare and
 * MATLAB code in this file `SW2007`, except for the input data.
 */

/*
 * Copyright (C) 2007-2013 Frank Smets and Raf Wouters
 * Copyright (C) 2013-2015 Johannes Pfeifer
 * Copyright (C) 2026 Yujia Ming
 *
...
// max posterior likelihood to obtain the mode
// CSMINWEL: Newton-type optimization
estimation(optim=('MaxIter',200),datafile=usmodel_data,mode_compute=4,first_obs=1, presample=4,lik_init=2,prefilter=0,mh_replic=0,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2, nograph, nodiagnostic, tex) dy dc dinve labobs pinfobs dw robs;
//posterior_mode = oo_.posterior_mode; // do not run in recursive_forecasts.m
//save('usmodel_mode.mat', 'posterior_mode'); // do not run in recursive_forecasts.m


// do not run in recursive_forecasts.m
...
//write_latex_prior_table;  // do not run in recursive_forecasts.m

// shock_decomposition y; // do not run in recursive_forecasts.m